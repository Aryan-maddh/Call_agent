...
import { Search, Settings, Bell, Home, ChevronRight, Copy, Edit } from 'lucide-react';

const notificationTypes = [
  { id: 'order_status', label: 'Order Status' },
  { id: 'upcoming_sale', label: 'Upcoming Sale' },
  { id: 'special_offers', label: 'Special Offers' },
  { id: 'new_item', label: 'New Item Arrival' },
];
      ...
      <div className="px-6 py-2">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-8 border-b border-gray-100">
            ...
            <p className="text-[19px] text-[#5B5B5B] mt-1">Change to notification settings, the user will get the update</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#00485A]/80 text-white text-[18px] font-medium">
                  <th className="px-8 py-4 border-r border-white/20 uppercase">TYPE</th>
                  <th className="px-8 py-4 border-r border-white/20 uppercase text-center">EMAIL</th>
                  <th className="px-8 py-4 border-r border-white/20 uppercase text-center">BROWSER</th>
                  <th className="px-8 py-4 uppercase text-center">APP</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {notificationTypes.map((type, idx) => (
                  <tr key={idx} className="text-[18px] text-[#3D3D3D]">
                    <td className="px-8 py-6 border-r border-gray-100 font-normal">{type.label}</td>
                    <td className="px-8 py-6 border-r border-gray-100 text-center">
                      <div className="flex justify-center">
                        <div className="w-5 h-5 border-2 border-gray-300 rounded-sm cursor-pointer hover:border-[#00485A] transition-colors"></div>
                      </div>
                    </td>
                    <td className="px-8 py-6 border-r border-gray-100 text-center">
                      <div className="flex justify-center">
          ...
          <div className="p-8 border-t border-gray-100 flex justify-end items-center gap-6">
            <button className="bg-[#00485A] text-white px-8 py-3 rounded-md font-medium text-[17px] shadow-md hover:bg-[#003845] transition-all">
              Save Changes
            </button>
            <button className="border border-[#00485A] text-[#00485A] px-8 py-3 rounded-md font-medium text-[17px] shadow-sm hover:bg-gray-50 transition-all">
              Discard
            </button>
          </div>
        </div>
